...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # loop = asyncio.get_event_loop()
            # response = await loop.run_in_executor(thread_pool, generate_response, user_message)
            # await websocket.send_text(fast_json_dumps({"answer": response, "agent": None, "cart": persistent_cart}))
